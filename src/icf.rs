...
where
    P: Prg<NUM_BYTES, 2>,
{
    pub fn new(f: IntvFn, prg: P) -> Self {
        Self {
            f,
    ...
    pub fn gen(&self, rng: &mut ThreadRng) -> (IcShare, IcShare) {
        let s0s: [[u8; NUM_BYTES]; 2] = rng.random();

        let gamma = self.f.r_in + IntG::max();
        ...
        let q_prime = self.f.q + IntG::one();
        let a_p = self.f.p + self.f.r_in;
        let a_q = self.f.q + self.f.r_in;
        ...
        let ind_2 = IntG::from(a_p > self.f.p);
        let ind_3 = IntG::from(a_q_prime > q_prime);
        let ind_4 = IntG::from(a_q == IntG::max());

        let z1 = z0 + -(self.f.r_out + ind_1 + -ind_2 + ind_3 + ind_4);
    ...
    pub fn eval(&self, b: bool, k: &IcShare, x: IntG) -> IntG {
        ...
        let ind_1 = IntG::from(x > self.f.p);
